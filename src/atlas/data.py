...
def seed_predict_entropy(Xpool, ypool, seed_idx, seed=0):
    from tabpfn import TabPFNClassifier
    clf = TabPFNClassifier(random_state=seed)
    clf.fit(Xpool.iloc[seed_idx], ypool[seed_idx])
    p = np.clip(clf.predict_proba(Xpool)[:, 1], 1e-6, 1 - 1e-6)
    return -(p * np.log(p) + (1 - p) * np.log(1 - p))


def seed_split(ypool, frac=0.05, seed=0):
    idx = []
    for k in (0, 1):
...
def oof_entropy(X, y, n_splits=5, seed=0, ctx_cap=12000):
    ...
    for tr_i, va_i in StratifiedKFold(n_splits, shuffle=True,
        ...
        oof[va_i] = tabpfn_predict_proba(Xtr, ytr, X.iloc[va_i], seed=seed)
    p = np.clip(oof, 1e-6, 1 - 1e-6)
    return oof, -(p * np.log(p) + (1 - p) * np.log(1 - p))
